...
contract Fact is owned, mortal {
    ...
    // Initializes a new Fact by providing a `_type` (a Fact type value that
    // should be coherent with the conventions above) and a `_hash = sha3(owner, name)`.
    function Fact(
        bytes8 _type,
        bytes32 _hash
    ) {
